...
Можно поэтапно рассчитывать количество линков для каждой итерации, но у нас есть отличный ограничитель сверху заданный начальными постулатами. А именно: для передачи всего трафика в соседние коммутаторы, достаточно ликов в количестве в два раза меньшем чем количество подключенных хостов. Так как линки от хостов загружены, максимум на 50%.
 ...
 Почему бы не поработать с вариантами подключения серверов, так чтобы максимально увеличить число уникальных путей на каждом узле. Например, при традиционном включении, все хосты подключаются к одной паре Leaf, но тогда таблица маршрутов на одном линке хоста, будет аналогична другому. Значит нужно оин линк включить в один, общий коммутатор, а второй, в тот коммутатор, куда другие хосты, включенные в первый коммутатор, не включены. Получается таблица подключения.
...
network:
  ...
  tunnels:
    vxlan1000:
      ...
      local: 172.16.2.6
```
Применим настройки командой ```sudo netplan apply ```. 
...
Сервер SRV-6 остался подключен  только одним линком к leaf-2, и так как на leaf-2 данные могут приходить только через другие сервера Srv-4 и Srv-5, то получается, что они являются промежуточными узлами для Srv-6.
